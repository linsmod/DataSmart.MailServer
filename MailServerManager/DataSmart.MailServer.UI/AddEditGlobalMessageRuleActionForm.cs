...
{
	public class AddEditGlobalMessageRuleActionForm : Form
	{
		...
		private GlobalMessageRuleActionBase m_pActionData;
		...
		public AddEditGlobalMessageRuleActionForm(GlobalMessageRule rule)
		{
			this.m_pRule = rule;
		...
		public AddEditGlobalMessageRuleActionForm(GlobalMessageRule rule, GlobalMessageRuleActionBase action)
		{
			this.m_pRule = rule;
			...
			if (action.ActionType == GlobalMessageRuleActionType.AutoResponse)
			{
				AutoResponse autoResponse = (AutoResponse)action;
				this.m_pAutoResponse_From.Text = autoResponse.From;
				this.m_pAutoResponse_FullMEssage.Text = Encoding.UTF8.GetString(autoResponse.Message);
				this.m_pAction.SelectedIndex = 0;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.DeleteMessage)
			{
				DeleteMessage arg_8F_0 = (DeleteMessage)action;
				this.m_pAction.SelectedIndex = 1;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.ExecuteProgram)
			{
				ExecuteProgram executeProgram = (ExecuteProgram)action;
				this.m_pExecuteProgram_ProgramToExecute.Text = executeProgram.Program;
				this.m_pExecuteProgram_ProgramArguments.Text = executeProgram.ProgramArguments;
				this.m_pAction.SelectedIndex = 2;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.ForwardToEmail)
			{
				ForwardToEmail forwardToEmail = (ForwardToEmail)action;
				this.m_pForwardToEmail_Email.Text = forwardToEmail.EmailAddress;
				this.m_pAction.SelectedIndex = 3;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.ForwardToHost)
			{
				ForwardToHost forwardToHost = (ForwardToHost)action;
				this.m_pForwardToHost_Host.Text = forwardToHost.Host;
				this.m_pForwardToHost_HostPort.Value = forwardToHost.Port;
				this.m_pAction.SelectedIndex = 4;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.StoreToDiskFolder)
			{
				StoreToDiskFolder storeToDiskFolder = (StoreToDiskFolder)action;
				this.m_pStoreToDiskFolder_Folder.Text = storeToDiskFolder.Folder;
				this.m_pAction.SelectedIndex = 5;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.MoveToIMAPFolder)
			{
				MoveToImapFolder moveToImapFolder = (MoveToImapFolder)action;
				this.m_pMoveToIMAPFolder_Folder.Text = moveToImapFolder.Folder;
				this.m_pAction.SelectedIndex = 6;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.AddHeaderField)
			{
				AddHeaderField addHeaderField = (AddHeaderField)action;
				this.m_pAddHeaderField_FieldName.Text = addHeaderField.HeaderFieldName;
				this.m_pAddHeaderField_FieldValue.Text = addHeaderField.HeaderFieldValue;
				this.m_pAction.SelectedIndex = 7;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.RemoveHeaderField)
			{
				RemoveHeaderField removeHeaderField = (RemoveHeaderField)action;
				this.m_pRemoveHeaderField_FieldName.Text = removeHeaderField.HeaderFieldName;
				this.m_pAction.SelectedIndex = 8;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.SendErrorToClient)
			{
				SendError sendError = (SendError)action;
				this.m_pSendErrorToClient_ErrorText.Text = sendError.SmtpErrorText;
				this.m_pAction.SelectedIndex = 9;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.StoreToFTPFolder)
			{
				StoreToFtp storeToFtp = (StoreToFtp)action;
				this.m_pStoreToFTPFolder_Server.Text = storeToFtp.Server;
				this.m_pStoreToFTPFolder_Port.Value = storeToFtp.Port;
				this.m_pStoreToFTPFolder_User.Text = storeToFtp.UserName;
				this.m_pStoreToFTPFolder_Password.Text = storeToFtp.Password;
				this.m_pStoreToFTPFolder_Folder.Text = storeToFtp.Folder;
				this.m_pAction.SelectedIndex = 10;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.PostToNNTPNewsGroup)
			{
				PostToNntpNewsgroup postToNntpNewsgroup = (PostToNntpNewsgroup)action;
				this.m_pPostToNNTPNewsgroup_Server.Text = postToNntpNewsgroup.Server;
				this.m_pPostToNNTPNewsgroup_Port.Value = postToNntpNewsgroup.Port;
				this.m_pPostToNNTPNewsgroup_Newsgroup.Text = postToNntpNewsgroup.Newsgroup;
				this.m_pAction.SelectedIndex = 11;
				return;
			}
			if (action.ActionType == GlobalMessageRuleActionType.PostToHTTP)
			{
				GlobalMessageRuleAction_PostToHttp globalMessageRuleAction_PostToHttp = (GlobalMessageRuleAction_PostToHttp)action;
				this.m_pPostToHTTP_URL.Text = globalMessageRuleAction_PostToHttp.Url;
		...
		private void InitializeComponent()
		{
			...
			this.m_pAction.Items.Add(new WComboBoxItem("Send Error To Client", GlobalMessageRuleActionType.SendErrorToClient));
		...
		private void m_Ok_Click(object sender, EventArgs e)
		{
			if (this.m_pAction.SelectedItem.ToString() == "Auto Response")
			{
				...
				{
					AutoResponse autoResponse = (AutoResponse)this.m_pActionData;
					autoResponse.Description = this.m_pDescription.Text;
					autoResponse.From = this.m_pAutoResponse_From.Text;
					autoResponse.Message = Encoding.UTF8.GetBytes(this.m_pAutoResponse_FullMEssage.Text);
					autoResponse.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Delete Message")
			{
				...
				{
					DeleteMessage deleteMessage = (DeleteMessage)this.m_pActionData;
					deleteMessage.Description = this.m_pDescription.Text;
			...
			else if (this.m_pAction.SelectedItem.ToString() == "Execute Program")
			{
				...
				{
					ExecuteProgram executeProgram = (ExecuteProgram)this.m_pActionData;
					executeProgram.Description = this.m_pDescription.Text;
					executeProgram.Program = this.m_pExecuteProgram_ProgramToExecute.Text;
					executeProgram.ProgramArguments = this.m_pExecuteProgram_ProgramArguments.Text;
					executeProgram.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Forward To Email")
			{
				...
				{
					ForwardToEmail forwardToEmail = (ForwardToEmail)this.m_pActionData;
					forwardToEmail.Description = this.m_pDescription.Text;
					forwardToEmail.EmailAddress = this.m_pForwardToEmail_Email.Text;
					forwardToEmail.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Forward To Host")
			{
				...
				{
					ForwardToHost forwardToHost = (ForwardToHost)this.m_pActionData;
					forwardToHost.Description = this.m_pDescription.Text;
					forwardToHost.Host = this.m_pForwardToHost_Host.Text;
					forwardToHost.Port = (int)this.m_pForwardToHost_HostPort.Value;
					forwardToHost.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Store To Disk Folder")
			{
				...
				{
					StoreToDiskFolder storeToDiskFolder = (StoreToDiskFolder)this.m_pActionData;
					storeToDiskFolder.Description = this.m_pDescription.Text;
					storeToDiskFolder.Folder = this.m_pStoreToDiskFolder_Folder.Text;
					storeToDiskFolder.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Move To IMAP Folder")
			{
				...
				{
					MoveToImapFolder moveToImapFolder = (MoveToImapFolder)this.m_pActionData;
					moveToImapFolder.Description = this.m_pDescription.Text;
					moveToImapFolder.Folder = this.m_pMoveToIMAPFolder_Folder.Text;
					moveToImapFolder.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Add Header Field")
			{
				...
				{
					AddHeaderField addHeaderField = (AddHeaderField)this.m_pActionData;
					addHeaderField.Description = this.m_pDescription.Text;
					addHeaderField.HeaderFieldName = this.m_pAddHeaderField_FieldName.Text;
					addHeaderField.HeaderFieldValue = this.m_pAddHeaderField_FieldValue.Text;
					addHeaderField.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Remove Header Field")
			{
				...
				{
					RemoveHeaderField removeHeaderField = (RemoveHeaderField)this.m_pActionData;
					removeHeaderField.Description = this.m_pDescription.Text;
					removeHeaderField.HeaderFieldName = this.m_pAddHeaderField_FieldName.Text;
					removeHeaderField.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Send Error To Client")
			{
				if (this.m_pSendErrorToClient_ErrorText.Text == "")
				{
					MessageBox.Show(this, "SMTP Error Text: value can't empty !", "Error:", MessageBoxButtons.OK, MessageBoxIcon.Hand);
					return;
				}
				if (!Net_Utils.IsAscii(this.m_pSendErrorToClient_ErrorText.Text))
				{
					MessageBox.Show(this, "SMTP Error Text: value can contain only printable ASCII chars !", "Error:", MessageBoxButtons.OK, MessageBoxIcon.Hand);
					return;
				}
				if (this.m_pActionData == null)
				{
					this.m_pActionData = this.m_pRule.Actions.Add_SendError(this.m_pDescription.Text, this.m_pSendErrorToClient_ErrorText.Text);
				}
				else
				{
					SendError sendError = (SendError)this.m_pActionData;
					sendError.Description = this.m_pDescription.Text;
					sendError.SmtpErrorText = this.m_pSendErrorToClient_ErrorText.Text;
					sendError.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Store To FTP Folder")
			{
				...
				{
					StoreToFtp storeToFtp = (StoreToFtp)this.m_pActionData;
					storeToFtp.Description = this.m_pDescription.Text;
					storeToFtp.Server = this.m_pStoreToFTPFolder_Server.Text;
					storeToFtp.Port = (int)this.m_pStoreToFTPFolder_Port.Value;
					storeToFtp.UserName = this.m_pStoreToFTPFolder_User.Text;
					storeToFtp.Password = this.m_pStoreToFTPFolder_Password.Text;
					storeToFtp.Folder = this.m_pStoreToFTPFolder_Folder.Text;
					storeToFtp.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Post To NNTP Newsgroup")
			{
				...
				{
					PostToNntpNewsgroup postToNntpNewsgroup = (PostToNntpNewsgroup)this.m_pActionData;
					postToNntpNewsgroup.Description = this.m_pDescription.Text;
					postToNntpNewsgroup.Server = this.m_pPostToNNTPNewsgroup_Server.Text;
					postToNntpNewsgroup.Port = (int)this.m_pPostToNNTPNewsgroup_Port.Value;
					postToNntpNewsgroup.Newsgroup = this.m_pPostToNNTPNewsgroup_Newsgroup.Text;
					postToNntpNewsgroup.Commit();
				}
			}
			else if (this.m_pAction.SelectedItem.ToString() == "Post To HTTP")
			{
				...
				{
					GlobalMessageRuleAction_PostToHttp globalMessageRuleAction_PostToHttp = (GlobalMessageRuleAction_PostToHttp)this.m_pActionData;
					globalMessageRuleAction_PostToHttp.Description = this.m_pDescription.Text;
					globalMessageRuleAction_PostToHttp.Url = this.m_pPostToHTTP_URL.Text;
					globalMessageRuleAction_PostToHttp.Commit();
				}
			}
