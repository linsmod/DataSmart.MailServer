...
{
	public class AddEditUserMessageRuleForm : Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.mt_Tab_General_Info.Text = "Specify user message rule info.";
		...
		private void m_pTab_Actions_ActionsToolbar_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			...
			{
				if (new AddEditUserMessageRuleActionForm(this.m_pRule)
				{
					Text = "Add/Edit User Message Rule Action"
			...
			else if (e.ClickedItem.Tag.ToString() == "edit")
			{
				...
				{
					UserMessageRuleActionBase action = (UserMessageRuleActionBase)this.m_pTab_Actions_Actions.SelectedItems[0].Tag;
					if (new AddEditUserMessageRuleActionForm(this.m_pRule, action)
					{
						Text = "Add/Edit User Message Rule Action"
					}.ShowDialog(this) == DialogResult.OK)
					{
			...
			{
				UserMessageRuleActionBase userMessageRuleActionBase = (UserMessageRuleActionBase)this.m_pTab_Actions_Actions.SelectedItems[0].Tag;
				if (MessageBox.Show(this, "Are you sure you want to delete Action '" + userMessageRuleActionBase.Description + "' !", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
				{
					userMessageRuleActionBase.Owner.Remove(userMessageRuleActionBase);
					this.LoadActions();
				}
			}
		}
		...
		{
			if (this.m_pTab_Actions_Actions.Items.Count > 0 && this.m_pTab_Actions_Actions.SelectedItems.Count > 0)
			{
				this.m_pTab_Actions_ActionsToolbar.Items[1].Enabled = true;
				this.m_pTab_Actions_ActionsToolbar.Items[2].Enabled = true;
				return;
			}
			this.m_pTab_Actions_ActionsToolbar.Items[1].Enabled = false;
			this.m_pTab_Actions_ActionsToolbar.Items[2].Enabled = false;
		}
		...
		private void LoadActions()
		{
			...
			foreach (UserMessageRuleActionBase userMessageRuleActionBase in this.m_pRule.Actions)
			{
				ListViewItem listViewItem = new ListViewItem();
				if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.AutoResponse)
				{
					listViewItem.Text = "Auto Response";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.DeleteMessage)
				{
					listViewItem.Text = "Delete Message";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.ForwardToEmail)
				{
					listViewItem.Text = "Forward To Email";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.ForwardToHost)
				{
					listViewItem.Text = "Forward To Host";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.StoreToDiskFolder)
				{
					listViewItem.Text = "Store To Disk Folder";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.ExecuteProgram)
				{
					listViewItem.Text = "Execute Program";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.MoveToIMAPFolder)
				{
					listViewItem.Text = "Move To IMAP Folder";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.AddHeaderField)
				{
					listViewItem.Text = "Add Header Field";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.RemoveHeaderField)
				{
					listViewItem.Text = "Remove Header Field";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.StoreToFTPFolder)
				{
					listViewItem.Text = "Store To FTP Folder";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.PostToNNTPNewsGroup)
				{
					listViewItem.Text = "Post To NNTP Newsgroup";
				}
				else if (userMessageRuleActionBase.ActionType == UserMessageRuleActionType.PostToHTTP)
				{
					listViewItem.Text = "Post To HTTP";
				}
				else
				{
					listViewItem.Text = userMessageRuleActionBase.ActionType.ToString();
				}
				listViewItem.Tag = userMessageRuleActionBase;
				listViewItem.SubItems.Add(userMessageRuleActionBase.Description);
