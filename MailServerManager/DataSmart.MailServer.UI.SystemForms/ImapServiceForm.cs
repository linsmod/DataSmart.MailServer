...
{
	public class ImapServiceForm : Form
	{
		...
		public ImapServiceForm(VirtualServer virtualServer)
		{
		...
		private void m_pTabGeneral_BindingsToolbar_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			...
			{
				AddEditBindInfoForm addEditBindInfoForm = new AddEditBindInfoForm(this.m_pVirtualServer.Server, true, WellKnownPorts.IMAP4, WellKnownPorts.IMAP4_SSL);
			...
			else if (e.ClickedItem.Tag.ToString() == "edit")
			{
				if (this.m_pTabGeneral_Bindings.SelectedItems.Count > 0)
				{
					...
					AddEditBindInfoForm addEditBindInfoForm2 = new AddEditBindInfoForm(this.m_pVirtualServer.Server, false, WellKnownPorts.IMAP4, WellKnownPorts.IMAP4_SSL, (IPBindInfo)listViewItem2.Tag);
		...
		private void LoadData()
		{
			try
			{
				IMAP_Settings iMAP = this.m_pVirtualServer.SystemSettings.IMAP;
				this.m_pEnabled.Checked = iMAP.Enabled;
				this.m_pGreetingText.Text = iMAP.GreetingText;
				this.m_pSessionTimeout.Value = iMAP.SessionIdleTimeOut;
				this.m_pMaxConnections.Value = iMAP.MaximumConnections;
				this.m_pMaxConnsPerIP.Value = iMAP.MaximumConnectionsPerIP;
				this.m_pMaxBadCommands.Value = iMAP.MaximumBadCommands;
				IPBindInfo[] binds = iMAP.Binds;
		...
		private void SaveData(bool confirmSave)
		{
			...
			{
				IMAP_Settings iMAP = this.m_pVirtualServer.SystemSettings.IMAP;
				iMAP.Enabled = this.m_pEnabled.Checked;
				iMAP.GreetingText = this.m_pGreetingText.Text;
				iMAP.SessionIdleTimeOut = (int)this.m_pSessionTimeout.Value;
				iMAP.MaximumConnections = (int)this.m_pMaxConnections.Value;
				iMAP.MaximumConnectionsPerIP = (int)this.m_pMaxConnsPerIP.Value;
				iMAP.MaximumBadCommands = (int)this.m_pMaxBadCommands.Value;
