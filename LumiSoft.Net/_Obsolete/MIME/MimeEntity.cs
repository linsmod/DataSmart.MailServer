...
namespace System.NetworkToolkit.Mime
{
	...
	public class MimeEntity
	{
		...
		{			
			// Write headers
			byte[] data = System.Text.Encoding.UTF8.GetBytes(FoldHeader(this.HeaderString));
		...
		public DateTime Date
		{
			get{ 
				if(m_pHeader.Contains("Date:")){
					try{
						return System.NetworkToolkit.MIME.MIME_Utils.ParseRfc2822DateTime(m_pHeader.GetFirst("Date:").Value);
					}
					catch{
			...
			set{ 
				if(m_pHeader.Contains("Date:")){
					m_pHeader.GetFirst("Date:").Value = System.NetworkToolkit.MIME.MIME_Utils.DateTimeToRfc2822(value);
				}
				else{
