    ...
    public class vCard
    {
        ...
        public static List<vCard> ParseMultiple(string file) {
            ...
            using (FileStream fs = File.OpenRead(file)) {
                TextReader r = new StreamReader(fs, System.Text.Encoding.UTF8);          
                while (line != null) {
                    line = r.ReadLine();
        ...
        {
            List<string> fileStrings = new List<string>();
            string[] fileStringArray = File.ReadAllLines(file,System.Text.Encoding.UTF8);
        ...
        public void Parse(FileStream stream) {
            List<string> fileStrings = new List<string>();
            string line = "";
            TextReader r = new StreamReader(stream,System.Text.Encoding.UTF8); 
            while (line != null) {
                line = r.ReadLine();
                fileStrings.Add(line);
            }
        ...
        public Image Photo
        {
            get{ 
                ...
                if(item != null){                    
                    return Image.FromStream(new MemoryStream(System.Text.Encoding.UTF8.GetBytes(item.DecodedValue)));
                }
                else{
                    return null;
                }
