...
namespace System.NetworkToolkit.TCP
{    
    /// <summary>
    ...
    public class TCP_Client : TCP_Session
    {
        ...
        public class ConnectAsyncOP : IDisposable,IAsyncOP
        {
            ...
            private TCP_Client                          m_pTCP_Client    = null;
            ...
            internal bool Start(TCP_Client owner)
            {
                ...
                try{
                    ...
                    if(m_pRemoteEP.AddressFamily == AddressFamily.InterNetwork){
                        ...
                        m_pSocket.ReceiveTimeout = m_pTCP_Client.m_Timeout;
                        m_pSocket.SendTimeout = m_pTCP_Client.m_Timeout;
                    }
                    ...
                    m_pTCP_Client.LogAddText("Connecting to " + m_pRemoteEP.ToString() + ".");
                    ...
                    if(m_pTCP_Client != null){
                        m_pTCP_Client.LogAddException("Exception: " + x.Message,x);
                    }
            ...
            private void BeginConnectCompleted(IAsyncResult ar)
            {
                try{
                    ...
                    m_pTCP_Client.LogAddText("Connected, localEP='" + m_pSocket.LocalEndPoint.ToString() + "'; remoteEP='" + m_pSocket.RemoteEndPoint.ToString() + "'.");

                    // Start SSL handshake.
                    if(m_SSL){
                        m_pTCP_Client.LogAddText("Starting SSL handshake.");
            ...
            private void BeginAuthenticateAsClientCompleted(IAsyncResult ar)
            {
                try{
                    ...
                    m_pTCP_Client.LogAddText("SSL handshake completed sucessfully.");
            ...
            {
                m_pTCP_Client.m_IsConnected = true;
                m_pTCP_Client.m_ID          = Guid.NewGuid().ToString();
                m_pTCP_Client.m_ConnectTime = DateTime.Now;
                m_pTCP_Client.m_pLocalEP    = (IPEndPoint)m_pSocket.LocalEndPoint;
                m_pTCP_Client.m_pRemoteEP   = (IPEndPoint)m_pSocket.RemoteEndPoint;
                m_pTCP_Client.m_pTcpStream  = new SmartStream(m_pStream,true);
                m_pTCP_Client.m_pTcpStream.Encoding = Encoding.UTF8;

                m_pTCP_Client.OnConnected(this.CompleteConnectCallback);
            }
        ...
        protected class SwitchToSecureAsyncOP : IDisposable,IAsyncOP
        {
            ...
            internal bool Start(TCP_Client owner)
            {
                ...
                try{
                    m_pSslStream = new SslStream(m_pTCP_Client.m_pTcpStream.SourceStream,false,this.RemoteCertificateValidationCallback);
            ...
            private void BeginAuthenticateAsClientCompleted(IAsyncResult ar)
            {
                try{
                    ...
                    // Close old stream, but leave source stream open.
                    m_pTCP_Client.m_pTcpStream.IsOwner = false;
                    m_pTCP_Client.m_pTcpStream.Dispose();

                    m_pTCP_Client.m_IsSecure = true;
                    m_pTCP_Client.m_pTcpStream = new SmartStream(m_pSslStream,true);
                }
                catch(Exception x){
                    m_pException = x;                    
                }
