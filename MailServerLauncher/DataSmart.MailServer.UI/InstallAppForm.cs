...
namespace DataSmart.MailServer.UI
{
    public class InstallAppForm : Form
    {
        ...
        private void m_pRunAsTryApp_Click(object sender, EventArgs e)
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Unix)
            {
                process = Process.Start("mono", Application.ExecutablePath + " -trayapp");
            }
            else
            {
                process = Process.Start(Application.ExecutablePath, "-trayapp");
            }

            this.Hide();
            this.ShowInTaskbar = false;
            var m_pInstallAsServiceEnabled = this.m_pInstallAsService.Enabled;
            var m_pUninstallServiceEnabled = m_pUninstallService.Enabled;
            ...
            //when exit process, restore the state of buttons
            process.WaitForExit();
            this.m_pInstallAsService.Enabled = m_pInstallAsServiceEnabled;
            this.m_pUninstallService.Enabled = m_pUninstallServiceEnabled;
            this.m_pRunAsTryApp.Enabled = true;
            this.m_pRunAsWindowsForm.Enabled = true;
            this.Show();
        ...
        private void m_pRunAsWindowsForm_Click(object sender, EventArgs e)
        {
            ...
            if (Environment.OSVersion.Platform == PlatformID.Unix)
            {
                process = Process.Start("mono", Application.ExecutablePath + " - winform");
            }
            else
            {
                process = Process.Start(Application.ExecutablePath, "-winform");
            }
            this.Hide();
            this.ShowInTaskbar = false;
            var m_pInstallAsServiceEnabled = this.m_pInstallAsService.Enabled;
